...
async function focusComposer(page) {
    // Do not use locator.focus() here. Messenger can replace the editor
    // between findVisibleEditable() and the focus action, which makes
    // Playwright wait on a locator that no longer points at the live editor.
    // Query and focus the current DOM node directly, with short retries.
    const selectors = [...editableSelectors, ...fallbackInputSelectors];

    for (let attempt = 1; attempt <= 5; attempt += 1) {
        const focused = await page.evaluate((candidateSelectors) => {
            const candidates = [];
    ...
    throw new Error('Live composer could not be focused.');
}
...
async function fillComposer(page, payload) {
    // Re-acquire the editor before every action. Messenger's Lexical editor
    // can expose contenteditable="true" while Playwright still reports the
    // node as not editable, which makes locator.fill() wait until timeout.
    // Focus + keyboard input works with the live editor and emits Lexical's
    // normal input events.
    for (let attempt = 1; attempt <= 3; attempt += 1) {
        ...
        try {
            ...
            // Clear any text left by a previous failed send without calling
            // locator.fill(), then type into the currently focused editor.
            ...
            const actualText = currentInput ? await getComposerText(currentInput) : '';
            if (actualText.trim() === payload.trim()) return;
        } catch (error) {
            if (attempt === 3) {
                throw new Error(`Composer input failed: ${error.message}`);
            }
            await page.waitForTimeout(250);
        }
    }
