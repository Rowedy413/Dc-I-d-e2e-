...
async function focusComposer(input) {
    // Do not use locator.click() here. Messenger sometimes puts an invisible
    // transition layer over the composer; click() then times out even though
    // the editor is present and usable.
...
async function sendMessage(page, payload, task) {
    ...
    let firstActionWorked = false;
    const sendButton = await findVisibleSendButton(page);
    if (sendButton) {
        // Force skips the same overlay/actionability issue that blocked the
        // composer click in the original version.
        firstActionWorked = await sendButton
            .click({ timeout: 3000, force: true })
            .then(() => true)
            .catch(() => false);
    }
    if (!firstActionWorked) {
        // Some Messenger layouts have no usable visible button; Enter is the
        // reliable keyboard fallback while the composer still has focus.
    ...
    if (currentInput) {
        const currentText = await getComposerText(currentInput);
        if (currentText.trim()) {
            await focusComposer(currentInput);
            const retryButton = await findVisibleSendButton(page);
            if (retryButton) {
                const retried = await retryButton
                    .click({ timeout: 3000, force: true })
                    .then(() => true)
                    .catch(() => false);
                if (!retried) await page.keyboard.press('Enter');
            } else {
                await page.keyboard.press('Enter');
            }
        }
    ...
    const remainingInput = await findVisibleEditable(page);
    if (remainingInput && (await getComposerText(remainingInput)).trim()) {
        throw new Error('Composer still contains text after Send/Enter fallback.');
    }
        ...
        while (task.isRunning) {
            ...
            try {
                ...
                if (recoveredInput && task.isRunning) {
                    try {
                        await focusComposer(recoveredInput);
                        await page.keyboard.insertText(payload);
                        await page.keyboard.press('Enter');
                        await page.waitForTimeout(300);
                        if ((await getComposerText(recoveredInput)).trim()) {
                            const recoveryButton = await findVisibleSendButton(page);
                            if (recoveryButton) {
                                await recoveryButton.click({ timeout: 3000, force: true });
                            }
                        }
                        if ((await getComposerText(recoveredInput)).trim()) {
                            throw new Error('Composer still contains text after recovery.');
                        }
